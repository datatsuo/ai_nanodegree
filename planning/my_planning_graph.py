...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            pre_nodes_a = node_a.prenodes
            # if all the prenodes for this action in the previous S level
            if pre_nodes_a.issubset(self.s_levels[level]):
                list_nodes_a.append(node_a)
                # connect the nodes in this new action level and the nodes in previous S level
                for node_s in self.s_levels[level]:
                    node_s.children.add(node_a)
                    node_a.parents.add(node_s)
        # add nodes to the build a new action level
        self.a_levels.append(list_nodes_a)
    ...
    def add_literal_level(self, level):
        ...
        list_nodes_s = []
        # for the action nodes in the previous action level
        for node_a in self.a_levels[level-1]:
            # for S nodes in the effnodes of the action node
            for node_s in node_a.effnodes:
                list_nodes_s.append(node_s)
                node_s.parents.add(node_a)
                node_a.children.add(node_s)
        # convert list to set and then add as a new literal S level
        self.s_levels.append(set(list_nodes_s))
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        test = False
        # if effect_add of node_a1 is in effect_rem of node_a2, return false
        for effect in node_a1.action.effect_add:
            if effect in node_a2.action.effect_rem:
                test = True
        # if effect_add of node_a2 is in effect_rem of node_a1, return false
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # four possibilities: to get True
        # (1) if one of effects to add for A1 matches with one of negative preconditions in A2
        # (2) if one of effects to remove for A1 matches with one of positive preconditions in A2
        # (3), (4): (1), (2) with A1 and A2 exchanged
        test = False
        for effect in node_a1.action.effect_add:
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        test = False
        # if one of the parent nodes (S nodes) of A1 is mutually exclusive
        # with one of the parent nodes (S nodes) of A2, then return True
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        test = False
        # if S1 is negative/positive and S2 is positive/negative and
        # if the literals for the two nodes match
        if((node_s1.is_pos != node_s2.is_pos) & (node_s1.symbol == node_s2.symbol)):
            test = True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        test = True
        # if one of the parent nodes (action nodes) of node_s1 is not mutex with
        # one of the parent nodes (action nodes) of node_s2, then return False
        for node_a1 in node_s1.parents:
            for node_a2 in node_s2.parents:
                if node_a1.is_mutex(node_a2) == False:
                    test = False
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            found = False
            # for each S level starting from level 0
            for level in range(len(self.s_levels)):
                # for nodes in that S level
                for node_s in self.s_levels[level]:
                    # if the goal matches with the literal of S node (for positive expression)
                    if((node_s.is_pos == True) & (goal == expr(node_s.symbol))):
                        level_sum += level
                        found = True
                        break
                    # if the goal matches with the literal of S node (for negative expression)
                    elif((node_s.is_pos == False) & (goal == expr('~{}'.format(node_s.symbol)))):
                        level_sum += level
                        found = True
                        break
                if found == True:
